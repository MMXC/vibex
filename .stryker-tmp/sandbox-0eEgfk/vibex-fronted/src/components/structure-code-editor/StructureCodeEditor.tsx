/**
 * Structure Code Editor Component
 * 编辑JSON配置，应用、撤销
 */
// @ts-nocheck
...
import styles from './StructureCodeEditor.module.css';

export interface StructureConfig {
  pages: Array<{ id: string; name: string; route: string; components: unknown[] }>;
}

export interface StructureCodeEditorProps {
  initialConfig: StructureConfig;
  onApply?: (config: StructureConfig) => void;
  onCancel?: () => void;
  readOnly?: boolean;
}

export function StructureCodeEditor({ initialConfig, onApply, onCancel, readOnly = false }: StructureCodeEditorProps) {
  ...
  const handleApply = useCallback(() => {
    try {
      const parsed = JSON.parse(code) as StructureConfig;
      setError(null);
      onApply?.(parsed);
    } catch (e) {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.toolbar}>
        <span className={styles.title}>页面结构配置 (JSON)</span>
...
export default StructureCodeEditor;
