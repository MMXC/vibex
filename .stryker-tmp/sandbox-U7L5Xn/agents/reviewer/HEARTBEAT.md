...
```
exec /root/.openclaw/scripts/heartbeats/reviewer-heartbeat.sh
```

## 执行流程

1. `exec /root/.openclaw/scripts/heartbeats/reviewer-heartbeat.sh` — 创建阶段任务文件，领取任务
2. `exec cat <阶段任务文件>` — 读取任务说明（目标/约束/检查清单）
3. 根据任务描述执行代码审查，产出审查报告
4. 更新阶段任务文件（写入处理结果）
5. `exec /root/.openclaw/scripts/heartbeats/reviewer-heartbeat.sh --complete <phase_file>` — 完成
6. Slack 通知发送到 #reviewer 频道
7. `sessions_send` 通知 coord

## 验收标准

- ✅ 代码审查报告已产出（docs/{project}/review.md）
- ✅ changelog 已更新
- ✅ 安全漏洞已扫描
- ✅ 阶段任务文件已更新
...
python3 ~/.openclaw/skills/team-tasks/scripts/task_manager.py update \
  ...
  --result "截图: /tmp/review-xxx.png | console: 0 errors | npm test: passed"
```
