# PRD: homepage-v4-fix-epic1-aipanel-test
...
**背景**: Epic1 AI 面板测试失败，根因是 Jest 配置错误——Jest 尝试执行 Playwright e2e 测试（ES Module 语法），导致 241 个测试套件失败。
...
|----------|------|----------|-----|
| ST-1.1 | 修改 jest.config.ts 排除 e2e | `expect(jest).not.toRunE2E()` | `testPathIgnorePatterns` 包含 `/e2e/` |
| ST-1.2 | 验证 Jest 运行正常 | `expect(exitCode).toBe(0)` | `npx jest` 无配置错误 |
