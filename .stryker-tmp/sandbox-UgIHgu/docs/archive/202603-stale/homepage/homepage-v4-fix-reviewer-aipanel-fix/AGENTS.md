# 开发约束: homepage-v4-fix-reviewer-aipanel-fix
...
- [ ] `testPathIgnorePatterns` 包含 `/\.spec\./` 和 `/\.e2e\./` 正则
- [ ] `pnpm test` 退出码为 0
- [ ] `pnpm test:e2e` 正常运行
...
| Story | 验收标准 |
|-------|----------|
| ST-1.1 | `testPathIgnorePatterns` 包含 `/\.spec\./` 和 `/e2e/` |
