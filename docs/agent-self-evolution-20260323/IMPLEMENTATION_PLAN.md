...
| ID | 任务 | 负责 | 验收标准 |
|----|------|------|---------|
| T6.1 | 实现 ActionBar 7 按钮业务逻辑 | dev | expect(buttons).toHaveBusinessLogic() (非空函数) |
| T6.2 | 按钮状态管理 | dev | expect(buttonState).toBeManaged() |
| T6.3 | ActionBar 单元测试 | tester | expect(7).toBe(7) with correct handlers |
...
| ID | 任务 | 负责 | 验收标准 |
|----|------|------|---------|
| T6.4 | 实现 BottomPanel 4 快捷功能 | dev | expect(shortcuts).toHaveBusinessLogic() |
| T6.5 | 实现 AIPanel 发送/关闭功能 | dev | expect(send).toTrigger() && expect(close).toWork() |
| T6.6 | 集成 `useHomeGeneration` hook | dev | expect(hook).toBeUsed() consistently |
...
| ID | 任务 | 负责 | 验收标准 |
|----|------|------|---------|
| T6.7 | 首页全流程 E2E 测试 | tester | expect(homePage).toHaveFullWorkflow() |
| T6.8 | ReactFlow 与首页集成测试 | tester | expect(integration).toWork() |
| T6.9 | 最终审查 | reviewer | 代码审查通过 |
| T6.10 | 发布验收报告 | pm | 验收标准 100% 通过 |

**Sprint 6 验收清单**:
- [ ] ActionBar 7 个按钮有实际业务逻辑
- [ ] BottomPanel 4 个快捷功能可用
- [ ] AIPanel 发送/关闭正常
- [ ] `useHomeGeneration` hook 统一处理
- [ ] E2E 测试 100% 通过
