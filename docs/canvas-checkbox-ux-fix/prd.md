...
| toggle 行为 | confirmContextNode（单向）| 双向 toggle |
...
| Story | 描述 | 工时 | 验收标准 |
|-------|------|------|---------|
| E1-S1 | 合并双 checkbox 为 1 个 toggle | 0.5h | expect(checkboxToggle).toBe(true) |
...
| F1.4 | 无 confirmedBadge | 确认状态通过 border 颜色表达 | expect(confirmedBadgeCount).toBe(0) | ✅ |
...
| Story | 描述 | 工时 | 验收标准 |
|-------|------|------|---------|
| E2-S1 | checkbox 前移到标题前 | 0.25h | expect(checkboxBeforeTitle).toBe(true) |
...
BusinessFlowTree 已正确实现，无需修改。

**功能点**:

| ID | 功能点 | 描述 | 验收标准 | 页面集成 |
|----|--------|------|----------|----------|
| F3.1 | 现状保持 | checkbox 在 header 内 inline | 无需修改 | ✅ |
...
| Story | 描述 | 工时 | 验收标准 |
|-------|------|------|---------|
| E4-S1 | 清理废弃样式 | 删除 .nodeTypeBadge / .confirmedBadge 等 | expect(deadCSSRemoved).toBe(true) |
...
| AC1.2 | 点击 BoundedContextTree checkbox | 切换 | checked 状态切换 |
