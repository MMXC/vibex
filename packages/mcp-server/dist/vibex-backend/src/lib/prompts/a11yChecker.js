...
exports.checkA11yCompliance = checkA11yCompliance;
// =============================================================================
// Constants
// =============================================================================
 ...
 */
function checkImageAlt(node, results) {
    ...
    if (!IMAGE_TYPES.some((t) => type === t))
        return;
    const alt = node.alt;
    const ariaLabel = node['aria-label'];
 ...
 */
function checkInteractiveKeyboardHint(node, results) {
    const nodeId = node.id ?? 'unknown';
    const nodeName = node.name;
    const type = node.type?.toLowerCase() ?? '';
    ...
    const ariaLabel = node['aria-label'];
    const title = node.title;
    const keyboardHint = node.keyboardHint;
 ...
 */
function checkColorContrast(node, results) {
    const nodeId = node.id ?? 'unknown';
    const nodeName = node.name;
    ...
    if (!bg.startsWith('#') || !fg.startsWith('#'))
        return;
    if (bg.length !== 4 && bg.length !== 7 && bg.length !== 9)
        return;
    if (fg.length !== 4 && fg.length !== 7 && fg.length !== 9)
        return;
    try {
 ...
 */
function checkAriaLabel(node, results) {
    const nodeId = node.id ?? 'unknown';
    const nodeName = node.name;
    const type = node.type?.toLowerCase() ?? '';
    const role = node.role;
    // Skip nodes that already have a name (from name or aria-label)
    if (nodeName || node['aria-label'])
        return;
 ...
 */
function checkA11yCompliance(nodes) {
    const issues = [];
