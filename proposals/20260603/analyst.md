# VibeX Sprint60 提案分析报告

**Sprint**: Sprint60
**日期**: 2026-06-03
**分析师**: coord (heartbeat self-implement — agent-submit phantom ghost, analyst agent never spawned)
...
- Git 工作流（cherry-pick/merge）成熟，双-CHANGELOG 同步机制确立
- CMD+K/L 全局快捷键框架就绪

---

## Sprint60 提案 (P001-P005)

### P001: 画布版本历史 UI 增强 — 快照时间线与分支视图

**问题描述**: S58-E1 实现了 canvasHistoryStore snapshot 持久化 + HistoryPanel 基本 UI，但缺少快照时间线视图和分支切换能力。用户无法直观浏览多个快照、比较差异或快速回滚到历史版本。

**根因分析**: S58-E1 的 HistoryPanel 只有列表视图，没有"快照分支"概念。IndexedDB snapshots 表支持多快照，但 UI 只展示 flat 列表。`compareSnapshots()` diff 逻辑也未集成到 UI。

**影响评估**: P0 — 版本分支是 VibeX 差异化核心功能，S58 打下数据层基础后，S60 必须完成 UI 层最后一公里。

**技术方案**:
1. 扩展 `HistoryPanel.tsx` — 新增"时间线视图" Tab，展示快照创建时间轴（垂直时间线，节点可点击预览）
...
### P002: 批量操作增强 — 画布批量删除/重命名 + 卡片批量操作面板

**问题描述**: 当前 VibeX 没有画布级别的批量管理能力。用户画布列表（CanvasListPanel）无法批量删除、重命名或导出多个画布。S57-E4 曾计划 batchDelete/batchRename 但未交付。

**根因分析**: S57-E4 的 batchDelete/batchRename 逻辑在 S57 自检中发现未完成（UI 面板缺失）。`canvasStore.ts` 有 `batchDeleteCanvas` 和 `renameCanvas` 方法，但用户没有操作入口。

**影响评估**: P1 — 影响重度用户的日常管理效率。个人工作区有 50+ 画布时，单个删除/重命名非常繁琐。
...
### P003: 协作增强 — 活动流 + 在线状态指示

**问题描述**: S42 开始做 presence/cursor，但协作者之间的"活动流"缺失。用户不知道协作者在编辑什么节点、什么时候做了操作。同时在线用户的实时在线状态也不直观。

**根因分析**: `presenceStore.ts` 有 `updateCursor()` 但没有"活动事件"概念。WebSocket 只广播 cursor 位置，没有结构化的 activity feed。`RemoteCursor.tsx` 也没有明确的在线/离线状态指示。

**影响评估**: P1 — 协作场景下的 awareness（情境感知）缺失，用户体验接近"单人在用"而非"多人实时协作"。

**技术方案**:
1. 新增 `activityStore.ts` — 协作者活动状态管理（最近操作类型、时间戳、影响节点）
2. 扩展 `wsCollaborationHandler.ts` — 新增 `activity:update` 消息类型
3. 新增 `ActivityFeed.tsx` — 协作者最近活动面板（RecentActivityPanel，DDSSidePanel 内）
4. 扩展 `RemoteCursor.tsx` — 在线状态指示器（绿色脉冲点 = 在线，灰色 = 空闲>5min）
5. `presenceStore.ts` 新增 `lastActivity` 字段

**验收标准**:
- [ ] ActivityFeed 面板展示最近 5 条协作者活动（编辑/移动/删除节点）
...
### P004: 画布导出增强 — 多格式批量导出 + PNG/PDF 单画布导出

**问题描述**: S58-S59 实现了模板批量导入导出，但画布本身的导出能力很弱。只能导出 `.vibex`/`.json` 格式，缺少用户常用的 PNG、PDF 格式。同时无法一次性导出多个画布。

**根因分析**: `ZipExporter.ts` 已有批量导出基础设施（S51-S52 期间实现）。需要扩展支持 PNG/PDF，并打通从 CanvasListPanel 一键批量导出的 UI 链路。

**影响评估**: P1 — 画布导出是高频用户需求，PNG 用于分享，PDF 用于归档。没有批量导出严重限制了大工作流用户的效率。

**技术方案**:
1. 扩展 `ZipExporter.ts` — 新增 `exportMultipleAsPNG(zip)` 和 `exportMultipleAsPDF(zip)` 方法
2. 使用 `html-to-image` 库（已在 S52 验证）进行 PNG 截取
...
- [ ] 导出过程有进度条（使用 S58-E4 ExportProgress 模式）
- [ ] vitest 测试: ZipExporter PNG/PDF 导出方法

---

### P005: 搜索体验增强 — 搜索历史 + 模糊匹配 + 搜索结果高亮优化

**问题描述**: S59-E1 实现了 Cmd+K 搜索面板，但搜索历史没有保存、模糊匹配不够智能、高亮效果不够明显。S59-E1 用的是 Fuse.js 精确匹配，"搜索历史"功能完全缺失。

**根因分析**: `canvasSearchStore.ts` 有 `searchResults` 但没有 `searchHistory` 概念。`fullTextSearch.ts` 使用 Fuse.js threshold=0.3 但对中文支持有限。没有保存用户最近的搜索关键词。

**影响评估**: P2 — 搜索是日常高频操作，体验细节影响使用愉悦度。S59-E1 打下基础，S60 打磨体验。

**技术方案**:
1. 扩展 `canvasSearchStore.ts` — 新增 `searchHistory`（最近 10 条，localStorage 持久化）、`addToHistory(query)`、`clearHistory()`
2. 优化 `fullTextSearch.ts` — 降低 Fuse.js threshold=0.2 提高精确度；中文分词支持（使用 `jieba` 或内置）
...
| 优先级 | 提案 | 理由 |
|--------|------|------|
| P0 | P001 画布版本历史 UI | S58 数据层已完成，UI 最后一公里是核心差异化功能 |
| P1 | P002 批量操作 | 严重影响日常管理效率，有 store 方法但缺 UI |
| P1 | P003 协作活动流 | S42 协作基础已完成，activity feed 是自然演进 |
| P1 | P004 画布导出增强 | 高频用户需求，ZipExporter 基础设施已就绪 |
| P2 | P005 搜索体验增强 | S59-E1 刚交付，体验打磨可分阶段 |
