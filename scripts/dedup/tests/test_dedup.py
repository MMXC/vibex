...
class TestExtractKeywords:
    ...
    def test_short_word_filter(self):
        """测试短词过滤（长度 < 2）"""
        kw = extract_keywords("a ab abc xyz")
        assert "a" not in kw   # len=1, filtered
        assert "ab" in kw       # len=2, kept (min=2)
        assert "abc" in kw      # len=3, passes
    ...
    def test_mixed_chinese_english(self):
        """测试中英混合（英文3+chars通过，中文bigrams被len>=2保留）"""
        kw = extract_keywords("API接口开发")
        assert "api" in kw   # English: 3+ chars
        assert "接口" in kw  # Chinese bigram preserved with len >= 2
