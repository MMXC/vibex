...
- Name: homepage.spec.ts >> Homepage Accessibility >> homepage has zero critical violations
- Location: vibex-fronted/tests/a11y/homepage.spec.ts:6:7
...
```ts
  ...
  4  | 
  5  | test.describe('Homepage Accessibility', () => {
  6  |   test('homepage has zero critical violations', async ({ page }) => {
  7  |     await page.goto('/');
  8  |     const result = await runAxe(page);
  9  | 
  10 |     if (!result.pass) {
  11 |       console.log(
  12 |         'Violations:',
  13 |         JSON.stringify(result.criticalViolations, null, 2)
  14 |       );
  15 |     }
  16 | 
  17 |     expect(
  18 |       result.criticalViolations,
  19 |       `Found ${result.criticalViolations.length} critical violations`
> 20 |     ).toHaveLength(0);
     |       ^ Error: Found 2 critical violations
  21 |   });
  22 | });
