...
严格 JSON 数组格式输出：
[
  {"name": "课程列表页", "flowId": "flow-abc123", "contextId": "ctx-cmnku0yelrffpxlax", "type": "list", "apis": [{"method": "GET", "path": "/api/courses", "params": []}]},
  {"name": "创建课程表单", "flowId": "flow-abc123", "contextId": "ctx-cmnku0yelrffpxlax", "type": "form", "apis": [{"method": "POST", "path": "/api/courses", "params": ["title", "description"]}]}
]`;
...
export async function POST(
  ...
  try {
    ...
    const flowSummary = flows
      .map(
        (f, idx) =>
          `- ${f.id || `flow-${idx + 1}`}: ${f.name} (上下文: ${f.contextId}): ${(f.steps || [])
            .map((s) => s.name)
            .join(' → ')}`
      )
      .join('\n');
