...
import { setComponentNodes as canvasSetComponentNodes } from '@/lib/canvas/canvasStore';
...
export function BusinessFlowTree({ readonly = false, isActive = true }: BusinessFlowTreeProps) {
  ...
  const editFlowNode = useFlowStore((s) => s.editFlowNode);
  const deleteFlowNode = useFlowStore((s) => s.deleteFlowNode);
  const confirmStep = useFlowStore((s) => s.confirmStep);
  const editStep = useFlowStore((s) => s.editStep);
  ...
  const selectAllNodes = useContextStore((s) => s.selectAllNodes);
