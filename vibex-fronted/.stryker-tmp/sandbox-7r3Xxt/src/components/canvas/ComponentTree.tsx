...
export function ComponentTree({ readonly = false, isActive: _isActive = true }: ComponentTreeProps) {
  ...
  const addComponentNode = useComponentStore((s) => s.addComponentNode);
  ...
  // E3-F2: Multi-select state
  const selectedNodeIds = useComponentStore((s) => s.selectedNodeIds);
  const toggleNodeSelect = useComponentStore((s) => s.toggleNodeSelect);
  const selectAllNodes = useComponentStore((s) => s.selectAllNodes);
  const clearNodeSelection = useComponentStore((s) => s.clearNodeSelection);
  ...
  const handleClearCanvas = useCallback(() => {
    if (window.confirm('确定清空画布？所有组件将被删除。')) {
      useComponentStore.getState().clearComponentCanvas();
    }
  }, []);
