...
export function ProjectBar({
  ...
  const handleUndo = useCallback(() => {
    ...
    const canvasStore = useContextStore.getState();

    // Priority: context > flow > component
    if (historyStore.canUndo('context')) {
      const prev = historyStore.undo('context');
      if (prev) canvasSetContextNodes(prev as typeof canvasStore.contextNodes);
      return;
    }
    if (historyStore.canUndo('flow')) {
      const prev = historyStore.undo('flow');
      if (prev) canvasSetFlowNodes(prev as typeof canvasStore.flowNodes);
      return;
    }
    if (historyStore.canUndo('component')) {
      const prev = historyStore.undo('component');
      if (prev) canvasSetComponentNodes(prev as typeof canvasStore.componentNodes);
    }
  }, []);
  ...
  const handleRedo = useCallback(() => {
    ...
    if (historyStore.canRedo('context')) {
      ...
      if (next) canvasSetContextNodes(next as typeof canvasStore.contextNodes);
      return;
    }
    if (historyStore.canRedo('flow')) {
      const next = historyStore.redo('flow');
      if (next) canvasSetFlowNodes(next as typeof canvasStore.flowNodes);
      return;
    }
    if (historyStore.canRedo('component')) {
      const next = historyStore.redo('component');
      if (next) canvasSetComponentNodes(next as typeof canvasStore.componentNodes);
    }
  }, []);
