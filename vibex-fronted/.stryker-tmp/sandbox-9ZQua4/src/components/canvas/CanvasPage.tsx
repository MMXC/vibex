...
export function CanvasPage({ useTabMode = false }: CanvasPageProps) {
  ...
  const handleConflictUseServer = useCallback(() => {
    ...
    if (serverData.contexts) {
      store.setContextNodes(serverData.contexts as Parameters<typeof store.setContextNodes>[0])
    }
    if (serverData.flows) {
      store.setFlowNodes(serverData.flows as Parameters<typeof store.setFlowNodes>[0])
    }
    if (serverData.components) {
      store.setComponentNodes(serverData.components as Parameters<typeof store.setComponentNodes>[0])
    }
    clearConflict()
  ...
  const handleConflictMerge = useCallback(() => {
    ...
    const localContexts = store.contextNodes
    const localFlows = store.flowNodes
    const localComponents = store.componentNodes
    ...
    store.setContextNodes(mergedContexts)
    store.setFlowNodes(mergedFlows)
    store.setComponentNodes(mergedComponents)
    clearConflict()
