...
export const Field = forwardRef<HTMLDivElement, FieldProps>(
  (
    ...
    const touched = isInForm
      ? formContext.touched
      : (controlledTouched ?? internalTouched);
    const error = isInForm
      ? formContext.error
      : (controlledError ?? internalError);

    // Validation function
    const validate = useCallback(
      (val: unknown, fieldRules?: ValidationRule[]): string => {
    ...
    return (
      <FieldContext.Provider value={fieldContextValue}>
        ...
        >
          ...
          {(hasError || hint) && (
            <span
              className={`${styles.hint} ${hasError ? styles.errorHint : ''}`}
            >
              {hasError ? error : hint}
            </span>
          )}
