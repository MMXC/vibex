...
- **E3 miniMapUtils.ts**: `src/lib/canvas/miniMapUtils.ts` — `getMiniMapNodeColor()` (user-story蓝/bounded-context紫/flow-step绿红灰) + `sampleNodesForMiniMap()`
- **E3 MiniMapPanel.tsx**: `nodeColor={getMiniMapNodeColor}` 替换静态蓝色
- **E3 vitest**: `src/lib/canvas/__tests__/miniMapUtils.test.ts` — 18 cases ✅ 18/18 PASS
