...
- **E1.1**: IndexedDB `notifications` objectStore + 7 new DB functions in `historyDB.ts`; `notificationStore` calls IDB on add
...
- **E1.3**: `fetchUnreadFromServer` calls GET `/api/notifications?unread=true&userId=...` + merges + saves to IndexedDB
- **E1.4**: `loadFromIndexedDB` rehydrates notifications from IndexedDB on store init
- **E1.5**: `getUnreadCountWithServer` aggregates local + server unread counts
- **E1.6**: `clearAll` calls `clearNotificationsFromDB`
- **E1.7**: DoD field coverage — all `NotificationEntry` fields present
- **E1.8**: `markAllAsRead` marks all as read + resets server count
