...
const HISTORY_FILE = path.join(__dirname, '..', 'coverage-history', 'coverage-history.jsonl');
...
function main() {
  ...
  // 检查阈值
  const minCoverage = Math.min(coverage.lines, coverage.branches, coverage.functions, coverage.statements);
  ...
  if (minCoverage < THRESHOLD) {
    ...
    if (degradation) {
      ...
      console.log(`   Change: ${degradation.diff > 0 ? '+' : ''}${degradation.diff}%`);
    }
