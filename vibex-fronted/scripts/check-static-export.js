...
const DYNAMIC_PATTERNS = [
  ...
  { pattern: /\[\[[^\]]+\]\]/, reason: 'Optional catch-all route (e.g., [[slug]])' },
]
...
function main() {
  ...
  if (dynamicCount > 0) {
    console.log('❌ Error: Found dynamic routes that cannot be statically exported.')
    console.log('   These routes require runtime data and will not work with `next export`.')
    process.exit(1)
  }
  
  if (warningCount > 0) {
    console.log('⚠️  Warning: Some routes may have issues with static export.')
    console.log('   Review the warnings above and ensure proper runtime handling.')
  }
