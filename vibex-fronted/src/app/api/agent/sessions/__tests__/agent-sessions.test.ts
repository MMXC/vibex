 ...
 * Uses vitest (with jest compat) to mock fetch and agentStore.
 *
 * Run: cd vibex-fronted && pnpm exec vitest run src/app/api/agent/sessions/__tests__/agent-sessions.test.ts
 */
...
const mockUseAgentStore = {
  getState: vi.fn(() => ({
    sessions: [] as Array<{ sessionKey: string }>,
...
// ── Mock fetch ───────────────────────────────────────────────────────────────
// Each test rebuilds the mock so calls can be recorded and responses controlled.
...
// Rebuild and replace globalThis.fetch before each test
let fetchMock: ReturnType<typeof buildFetchMock>;
globalThis.fetch = buildFetchMock();
fetchMock = globalThis.fetch as ReturnType<typeof buildFetchMock>;
...
describe('GET /api/agent/sessions', () => {
  ...
  it('proxies to correct backend URL', async () => {
    ...
    expect(recordedCalls.some((c) => c.url.includes('/api/agent/sessions'))).toBe(true);
  });
...
describe('POST /api/agent/sessions', () => {
  ...
  it('returns 201 with sessionKey for valid task', async () => {
    ...
    const data = await res.json() as { sessionKey: string; status: string };
