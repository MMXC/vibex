...
describe('Chat (/chat)', () => {
  ...
  it('renders with API response', async () => {
    mockGetMessages.mockResolvedValueOnce([
      { id: '1', role: 'user', content: 'Hello', projectId: '', createdAt: new Date().toISOString() },
      { id: '2', role: 'assistant', content: 'Hi there!', projectId: '', createdAt: new Date().toISOString() }
    ])
    render(<Chat />)
  ...
  it('handles clear conversation', async () => {
    ...
    const newChatButton = Array.from(buttons).find(b => b.textContent?.includes('新对话'))
    if (newChatButton) {
      await act(async () => {
  ...
  it('renders background effect elements', () => {
    const { container } = render(<Chat />)
    const bgElements = container.querySelectorAll('[class*="bgEffect"], [class*="gridOverlay"]')
