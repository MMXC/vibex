...
jest.mock('@/services/api', () => ({
  apiService: {
    ...
    generateFlow: jest.fn().mockResolvedValue({ 
      nodes: [{ id: '1', position: { x: 100, y: 100 }, data: { label: 'AI Generated' } }], 
      edges: [] 
    }),
  }
}))
...
describe('Flow (/flow)', () => {
  ...
  it('has export functionality', () => {
    render(<FlowEditor />)
    // 检查导出相关元素
    const exportElements = document.querySelectorAll('[class*="export"], button[title*="导出"]')
  ...
  it('handles AI modal input', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByPlaceholderText(/创建一个用户登录流程/)).toBeInTheDocument()
    })
