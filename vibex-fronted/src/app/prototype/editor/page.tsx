...
// 模拟流式响应的回复
const mockResponses = [
  "我来帮你分析这个原型设计。根据当前页面结构，这是一个典型的后台管理系统导航页面。",
  "页面包含以下核心组件：\n- 顶部导航栏 (navigation)\n- 标题文字 (text/h1)\n- 导航链接项",
  "建议优化：\n1. 可以增加移动端响应式适配\n2. 导航项可以添加图标\n3. 考虑增加面包屑导航",
  "这个设计风格简约现代，适合企业级应用。配色方案使用了深色系护眼设计。",
]
...
function InteractiveRenderer({ 
  ...
  const baseStyle: React.CSSProperties = useMemo(() => ({
    ...
    transform: hovered && !component.props?.disabled ? 'scale(1.02)' : 'scale(1)',
    boxShadow: hovered && !component.props?.disabled ? '0 4px 12px rgba(0, 212, 255, 0.3)' : 'none',
  ...
  switch (component.type) {
    ...
    case 'button':
      return (
        <button
          ...
          style={{
            ...baseStyle,
            padding: component.props?.size === 'small' ? '8px 16px' : component.props?.size === 'large' ? '16px 32px' : '12px 24px',
            backgroundColor: state.clicked ? '#00a8cc' : (component.props?.variant === 'primary' ? '#00d4ff' : component.props?.variant === 'danger' ? '#ef4444' : 'transparent'),
    ...
    case 'toggle':
      return (
        <div 
          key={component.id} 
          style={{ ...baseStyle, display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}
          onClick={handleClick}
        >
          <div style={{
            width: '48px',
          ...
          {component.props?.label && (
            <span style={{ color: '#e2e8f0', fontSize: '14px' }}>{component.props.label}</span>
          )}
        </div>
      )
      ...
      return (
        ...
        >
          {component.props?.title && (
            <h3 style={{ color: '#e2e8f0', fontSize: '18px', fontWeight: '600', marginBottom: '12px' }}>
              {component.props.title}
            </h3>
          )}
    ...
    case 'avatar':
      ...
      return (
        ...
        >
          {component.props?.src ? (
            <img src={component.props.src} alt={component.props.alt || ''} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ) : (
      ...
      return (
        <span
          ...
          style={{
            ...
            fontSize: '12px',
            fontWeight: '600',
            backgroundColor: badgeColors[component.props?.color || 'primary'] + '20',
        ...
        <div key={component.id} style={{ ...baseStyle }}>
          {component.props?.label && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ color: '#94a3b8', fontSize: '14px' }}>{component.props.label}</span>
              <span style={{ color: '#e2e8f0', fontSize: '14px' }}>{state.value || component.props?.value || 0}%</span>
            </div>
          )}
          <div style={{ width: '100%', height: '8px', backgroundColor: '#334155', borderRadius: '4px', overflow: 'hidden' }}>
            <div style={{
              ...
              height: '100%',
              backgroundColor: component.props?.color === 'success' ? '#22c55e' : '#00d4ff',
              borderRadius: '4px',
    ...
    default:
      return (
        <div 
          key={component.id} 
          style={{ ...baseStyle, padding: '12px', backgroundColor: '#1e293b', borderRadius: '8px', color: '#94a3b8' }}
          onClick={() => onComponentClick(component)}
        >
          {component.type}: {component.props?.text || component.props?.title || '未配置'}
        </div>
      )
  }
}
...
function EditorContent() {
  ...
  const [compareVersions, setCompareVersions] = useState<VersionCompare | null>(null)
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      role: 'assistant',
      content: '你好！我是 AI 助手，可以帮你分析和优化这个原型设计。有什么问题尽管问我！',
      timestamp: new Date()
    }
  ])
  ...
  const loadVersionHistory = useCallback(async () => {
    ...
    try {
      const snapshots = await apiService.getPrototypeSnapshots(projectId)
      // 转换为版本历史格式
      const versions: VersionHistory[] = snapshots.map((s: PrototypeSnapshot) => ({
        id: s.id,
        version: s.version || 1,
  ...
  // 回滚到指定版本
  const handleRollback = useCallback(async (version: VersionHistory) => {
    if (!projectId || !version.content) return
    
    if (!confirm(`确定要回滚到 "${version.name}" 吗？当前未保存的更改将会丢失。`)) {
      return
    }
  ...
  const handleSaveVersion = useCallback(async () => {
    ...
    const versionName = prompt('请输入版本名称：', `版本 ${versionList.length + 1}`)
    ...
    <div className={styles.page}>
      ...
      {lastInteraction && (
        <div className={styles.interactionToast}>
          ...
          <button className={styles.toastClose} onClick={() => setLastInteraction('')}>✕</button>
        </div>
      )}
      ...
      <div className={styles.mainContainer}>
            ...
            <div className={styles.versionList}>
              {loadingVersions ? (
                ...
                <div className={styles.versionEmpty}>
                  <span>暂无版本记录</span>
                  <span className={styles.versionEmptyHint}>点击"保存"创建第一个版本</span>
                </div>
              ) : (
                  ...
                  >
                    ...
                    <div className={styles.versionInfo}>
                      <div className={styles.versionName}>
                        {version.name}
                        {index === 0 && <span className={styles.currentBadge}>当前</span>}
                      </div>
        ...
        {compareModalOpen && compareVersions && (
          <div className={styles.compareModal} onClick={() => setCompareModalOpen(false)}>
              ...
              <div className={styles.compareVersions}>
                <div className={styles.compareVersionPanel}>
                  <div className={styles.compareVersionHeader}>
                    ...
                    <span className={styles.compareVersionName}>{compareVersions.left.name}</span>
                    <span className={styles.compareVersionTime}>
                      {new Date(compareVersions.left.createdAt).toLocaleString('zh-CN')}
                    </span>
                  </div>
                ...
                <div className={styles.compareVersionPanel}>
                  <div className={styles.compareVersionHeader}>
                    ...
                    <span className={styles.compareVersionName}>{compareVersions.right.name}</span>
                    <span className={styles.compareVersionTime}>
                      {new Date(compareVersions.right.createdAt).toLocaleString('zh-CN')}
                    </span>
                  </div>
                  <div className={styles.compareVersionContent}>
