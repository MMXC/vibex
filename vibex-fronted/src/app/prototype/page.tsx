...
const devices = [
  ...
  { id: 'tablet', name: '平板', width: '768px', icon: '📱', description: 'iPad / 平板设备' },
  { id: 'mobile', name: '手机', width: '375px', icon: '📲', description: 'iPhone / Android' },
]
...
export default function PrototypePreview() {
    ...
    const fetchData = async () => {
      try {
        if (projectId) {
          ...
          // 加载快照
          const snapshotsData = await apiService.getPrototypeSnapshots(projectId)
  ...
  // 渲染组件预览
  const renderComponent = useCallback((component: UIComponent, depth = 0): React.ReactNode => {
    ...
    switch (component.type) {
      case 'navigation':
        return (
          <nav 
            ...
            onClick={(e) => { e.stopPropagation(); setSelectedComponent(component); }}
          >
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              {(component.props.items as string[] || []).map((item, i) => (
                <span key={i} style={{ color: '#94a3b8', fontSize: '14px', cursor: 'pointer' }}>
                  {item}
                </span>
              ))}
            </div>
      ...
      case 'container':
        return (
          <div 
            ...
            style={{
              ...baseStyle,
              padding: component.props.className === 'hero' ? '80px 24px' : '24px',
              backgroundColor: component.props.className === 'hero' ? 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)' : 'transparent',
              textAlign: component.props.className === 'hero' ? 'center' : 'left',
            }}
            onClick={(e) => { e.stopPropagation(); setSelectedComponent(component); }}
          >
      ...
      case 'text':
        const TextTag = component.props.variant === 'h1' ? 'h1' : 
                       component.props.variant === 'h2' ? 'h2' : 'p'
        const textStyle: React.CSSProperties = {
          ...baseStyle,
          fontSize: component.props.variant === 'h1' ? '32px' : 
                    component.props.variant === 'h2' ? '24px' : '16px',
          fontWeight: component.props.variant?.startsWith('h') ? 'bold' : 'normal',
          color: component.props.className === 'hero-title' ? '#ffffff' : 
                 component.props.className === 'hero-subtitle' ? '#94a3b8' : '#e2e8f0',
      ...
      case 'button':
        const buttonStyle: React.CSSProperties = {
          ...baseStyle,
          padding: component.props.size === 'large' ? '16px 32px' : 
                   component.props.size === 'small' ? '8px 16px' : '12px 24px',
          backgroundColor: component.props.variant === 'primary' ? '#00d4ff' : 'transparent',
          color: component.props.variant === 'primary' ? '#0f172a' : '#00d4ff',
          border: component.props.variant === 'primary' ? 'none' : '1px solid #00d4ff',
          borderRadius: '8px',
        ...
        return (
          ...
          >
            ...
            {component.props.value && (
              <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#00d4ff', marginBottom: '8px' }}>
                {component.props.value}
              </div>
            )}
        ...
        return (
          <div 
            ...
            style={{
              ...baseStyle,
              display: 'grid',
              gridTemplateColumns: component.props.columns ? `repeat(${component.props.columns}, 1fr)` : '1fr',
              gap: '16px',
            }}
            onClick={(e) => { e.stopPropagation(); setSelectedComponent(component); }}
          >
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
                  {columns.map((col, i) => (
                    <th key={i} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '14px', fontWeight: 600, color: '#94a3b8', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    {row.map((cell, j) => (
                      <td key={j} style={{ padding: '12px 16px', fontSize: '14px', color: '#e2e8f0' }}>
                        {cell}
                      </td>
                    ))}
                  </tr>
        ...
        return (
          ...
          >
            <div style={{ textAlign: 'center', color: '#94a3b8' }}>
              ...
              <div style={{ fontSize: '14px' }}>{component.props.title || '图表'}</div>
            </div>
          </div>
        )
      ...
      default:
        return (
          <div 
            key={component.id}
            style={{ ...baseStyle, padding: '16px', backgroundColor: 'rgba(255,255,255,0.02)', borderRadius: '8px' }}
            onClick={(e) => { e.stopPropagation(); setSelectedComponent(component); }}
          >
            {component.type}
          </div>
    ...
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <div className={styles.headerLeft}>
          <Link href={projectId ? `/dashboard?projectId=${projectId}` : '/dashboard'} className={styles.backBtn}>
            <span>←</span>
            <span>返回</span>
          </Link>
            ...
            <div className={styles.browserFrame}>
              <div className={styles.browserHeader}>
                <div className={styles.browserDots}>
                  <span className={styles.browserDot} style={{ backgroundColor: '#ff5f56' }}></span>
                  <span className={styles.browserDot} style={{ backgroundColor: '#ffbd2e' }}></span>
                  <span className={styles.browserDot} style={{ backgroundColor: '#27ca40' }}></span>
                </div>
              ...
              <div className={styles.browserContent}>
                {selectedPage ? (
                  <div className={styles.pageContent}>
                    {selectedPage.components?.map(comp => renderComponent(comp))}
                  </div>
                ) : (
            ...
            <div className={styles.panelSection}>
              ...
              <div className={styles.infoGrid}>
                <div className={styles.infoItem}>
                  ...
                  <span className={styles.infoValue}>{selectedPage?.name || '-'}</span>
                </div>
                <div className={styles.infoItem}>
                  <span className={styles.infoLabel}>路由</span>
                  <span className={styles.infoValue}>{selectedPage?.route || '-'}</span>
                </div>
                <div className={styles.infoItem}>
                  <span className={styles.infoLabel}>组件数</span>
                  <span className={styles.infoValue}>{selectedPage?.components?.length || 0}</span>
                </div>
                <div className={styles.infoItem}>
                  <span className={styles.infoLabel}>布局</span>
                  <span className={styles.infoValue}>{selectedPage?.layout?.type || 'single'}</span>
                </div>
              </div>
            </div>
