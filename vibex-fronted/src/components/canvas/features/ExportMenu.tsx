...
export function ExportMenu({ label = '导出', disabled = false, onImportClick }: ExportMenuProps) {
  ...
  const formatOptions: { format: ExportFormat; label: string; description: string }[] = [
    ...
    { format: 'yaml', label: 'YAML', description: '可读性高的配置文件格式' },
    ...
    <div className={styles.exportMenuWrapper} ref={menuRef}>
      ...
      {isOpen && (
        <div className={styles.exportDropdown} role="menu" data-testid="export-dropdown">
          ...
          {/* E3-U3: Import section */}
          {onImportClick && (
            <div className={styles.batchSection}>
              <span className={styles.sectionLabel}>导入</span>
              <button
                type="button"
          ...
          <div className={styles.batchSection}>
            ...
            <div className={styles.batchFormatRow}>
              ...
              <label className={styles.batchFormatLabel}>
                <input
                  ...
                  name="batch-format"
                  value="pdf"
                  checked={selectedBatchFormat === 'pdf'}
                  onChange={() => setSelectedBatchFormat('pdf')}
                  className={styles.scopeRadio}
                />
                PDF
              </label>
            ...
            <button
              ...
              title="将所有节点导出为 PNG/SVG/PDF 文件并打包为 ZIP"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                <polyline points="7 10 12 15 17 10" />
