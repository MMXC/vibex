...
import { useAIAgentContext } from '@/hooks/ai/useAIAgentContext';
import { useAgentStore, type RetryMode } from '@/stores/dds/agentStore';
...
export const AIDraftDrawer = memo(function AIDraftDrawer({
  ...
  const [selectedRetryMode, setSelectedRetryMode] = useState<RetryMode>(defaultRetryMode);

  // Sync selected retry mode when default changes
  useEffect(() => {
    setSelectedRetryMode(defaultRetryMode);
  }, [defaultRetryMode]);
    ...
    async (userPrompt: string) => {
      ...
      try {
        ...
        if (err instanceof Error && err.name === 'AbortError') {
          setErrorMessage('连接中断，正在重连...');
          setIsRetrying(true); // E4: Show retry spinner on disconnect
        } else {
          setErrorMessage(
  ...
  const handleRetry = useCallback(() => {
    if (lastPromptRef.current) {
      ...
      setGeneratedCards([]);
      setErrorMessage(null);
      setIsRetrying(false); // E4: Clear retry spinner
  ...
  const handleRetryModeChange = useCallback(
    (e: ChangeEvent<HTMLSelectElement>) => {
      const mode = e.target.value as RetryMode;
    ...
    <>
      ...
      >
        ...
        <div className={styles.header}>
          ...
          {/* E4: Canvas Context Block */}
          <div className={styles.canvasContextBadge} data-testid="canvas-context-badge">
            📋 {canvasSummary}
          </div>
          {/* E4: Retry Config Dropdown */}
          <div className={styles.retryConfig}>
            <label htmlFor="retry-mode" className={styles.retryLabel}>重试:</label>
            <select
              id="retry-mode"
              className={styles.retrySelect}
              value={selectedRetryMode}
              onChange={handleRetryModeChange}
              aria-label="重试次数配置"
          ...
          <div className={styles.messageList} ref={messageListRef}>
            ...
            {isLoading && (
              <div className={styles.message} data-testid="loading-indicator">
                {isRetrying ? (
                  // E4: Retry spinner during reconnection
                  <div className={styles.retrySpinner} data-testid="retry-spinner">
                    <div className={styles.retrySpinnerDot} />
                    <span>重连中...</span>
                  </div>
                ) : (
