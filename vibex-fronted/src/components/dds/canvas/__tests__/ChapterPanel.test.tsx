...
describe('ChapterPanel', () => {
  ...
  // ==================== Snapshot Tests (F2.1-U1) ====================

/**
 * Stable snapshot seed — deterministic IDs for snapshot reproducibility.
 * Uses fixed offsets instead of Date.now() / Math.random().
 */
const SNAPSHOT_SEED = 1000;

function makeStableCard(index: number) {
  const titles = ['Story 1', 'Story 2', 'Story 3'];
  return {
    id: `snap-card-${index}`,
...
describe('ChapterPanel — Snapshot Tests (F2.1-U1)', () => {
  beforeEach(() => {
    setupStore();
    vi.clearAllMocks();
  });

  describe('toMatchSnapshot — empty states', () => {
    it('renders empty requirement chapter as snapshot', () => {
      const { asFragment } = render(<ChapterPanel chapter="requirement" />);
      expect(asFragment()).toMatchSnapshot();
    });

    it('renders empty context chapter as snapshot', () => {
      const { asFragment } = render(<ChapterPanel chapter="context" />);
      expect(asFragment()).toMatchSnapshot();
    });


    it('renders empty flow chapter as snapshot', () => {
      const { asFragment } = render(<ChapterPanel chapter="flow" />);
      expect(asFragment()).toMatchSnapshot();
    });
  });

  describe('toMatchSnapshot — with cards', () => {
    it('renders requirement chapter with 1 card as snapshot', () => {
      setupStore({
        chapters: {
          requirement: {
            type: 'requirement',
            cards: [makeStableCard(0)],
            edges: [],
            loading: false,
            error: null,
          },
    ...
    it('renders requirement chapter with 3 cards as snapshot', () => {
      setupStore({
        chapters: {
          requirement: {
            type: 'requirement',
            cards: [makeStableCard(0), makeStableCard(1), makeStableCard(2)],
            edges: [],
            loading: false,
            error: null,
          },
  ...
  describe('toMatchSnapshot — states', () => {
    it('renders loading state as snapshot', () => {
      setupStore({
        chapters: {
          requirement: { type: 'requirement', cards: [], edges: [], loading: true, error: null },
          context: { type: 'context', cards: [], edges: [], loading: false, error: null },
    ...
    it('renders error state as snapshot', () => {
      setupStore({
        chapters: {
          requirement: { type: 'requirement', cards: [], edges: [], loading: false, error: '加载失败，请重试' },
          context: { type: 'context', cards: [], edges: [], loading: false, error: null },
