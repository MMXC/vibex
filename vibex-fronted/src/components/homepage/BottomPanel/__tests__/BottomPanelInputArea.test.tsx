...
describe('BottomPanelInputArea', () => {
  describe('ST-6.2: 需求录入 TextArea (支持 5000 字)', () => {
    it('渲染 TextArea', () => {
      render(<BottomPanelInputArea onChange={vi.fn()} onSend={vi.fn()} />);
    ...
    it('粘贴 5000 字不崩溃', () => {
      ...
      render(<BottomPanelInputArea onChange={onChange} onSend={vi.fn()} />);
    ...
    it('字数超过 5000 时显示警告样式', () => {
      ...
      expect(textarea).toBeInTheDocument() // overLimit state checked via component rendering;
    });
    ...
    it('字数超过 5000 时统计显示错误样式', () => {
      ...
      expect(charCount).toBeInTheDocument() // charCountError state checked via component rendering;
    });
  ...
  describe('ST-6.3: 发送按钮', () => {
    ...
    it('点击发送按钮调用 onSend', () => {
      const onSend = vi.fn();
      render(<BottomPanelInputArea onChange={vi.fn()} onSend={onSend} />);
      fireEvent.change(screen.getByTestId('requirement-input'), { target: { value: 'test' } });
    ...
    it('disabled 状态下发送按钮禁用', () => {
      render(<BottomPanelInputArea onChange={vi.fn()} onSend={vi.fn()} disabled />);
      fireEvent.change(screen.getByTestId('requirement-input'), { target: { value: 'test' } });
      expect(screen.getByTestId('send-btn')).toBeDisabled();
    });

    it('isSending 状态下发送按钮禁用', () => {
      render(<BottomPanelInputArea onChange={vi.fn()} onSend={vi.fn()} isSending />);
      fireEvent.change(screen.getByTestId('requirement-input'), { target: { value: 'test' } });
      expect(screen.getByTestId('send-btn')).toBeDisabled();
    });
