...
describe('CollapseHandle', () => {
  describe('ST-6.1: 收起/展开手柄', () => {
    it('渲染默认展开状态', () => {
      render(<CollapseHandle isCollapsed={false} onToggle={vi.fn()} />);
    ...
    it('渲染收起状态', () => {
      render(<CollapseHandle isCollapsed={true} onToggle={vi.fn()} />);
      expect(screen.getByText('⬇️')).toBeInTheDocument();
    ...
    it('title 属性正确', () => {
      const { rerender } = render(<CollapseHandle isCollapsed={false} onToggle={vi.fn()} />);
      expect(screen.getByTestId('collapse-handle')).toHaveAttribute('title', '收起面板');

      rerender(<CollapseHandle isCollapsed={true} onToggle={vi.fn()} />);
