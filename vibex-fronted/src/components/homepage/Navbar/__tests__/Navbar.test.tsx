...
// =============================================================================
// Mocks — vi.mock calls are hoisted by Jest before imports
// =============================================================================
...
const mockAuthIsAuthenticated = vi.fn();
...
// Mock Toast
const mockShowToastFn = vi.fn();
const mockHideToastFn = vi.fn();
...
describe('Navbar', () => {
  ...
  describe('ST-2.1: Logo 显示', () => {
    it('显示 VibeX Logo', () => {
      render(<Navbar isAuthenticated={false} onLoginClick={vi.fn()} />);
    ...
    it('Logo 可自定义标题', () => {
      render(<Navbar isAuthenticated={false} onLoginClick={vi.fn()} title="CustomTitle" />);
  ...
  describe('ST-2.4: 登录后显示项目入口', () => {
    it('已登录显示 我的项目 链接', () => {
      render(<Navbar isAuthenticated={true} onLoginClick={vi.fn()} />);
  ...
  describe('className 传递', () => {
    it('支持自定义 className', () => {
      render(<Navbar isAuthenticated={false} onLoginClick={vi.fn()} className="custom-class" />);
  ...
  describe('F-2.1: 新画布按钮 auth guard', () => {
    ...
    it('新画布按钮有 data-testid="new-canvas-btn"', () => {
      render(<Navbar isAuthenticated={undefined} onLoginClick={vi.fn()} />);
