...
function FlowEditorInner({
  ...
  return (
    <div ref={wrapperRef} className={`${styles.container} ${className || ''}`} style={style}>
      <ReactFlow
        nodes={nodes}
        edges={edges}
...
export const flowUtils = {
   ...
   */
  generateId: (prefix: string = 'node'): string => `${prefix}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  getConnectedNodes: (nodeId: string, nodes: Node[], edges: Edge[]): Node[] => {
    ...
    return nodes.filter((node) => connectedEdgeIds.has(node.id) && node.id !== nodeId)
  },
}
