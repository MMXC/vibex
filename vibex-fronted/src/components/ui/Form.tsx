...
interface FormContextValue {
  ...
  validateField: (name: string, value: unknown, rules?: ValidationRule[]) => string;
...
export interface FieldProps {
  ...
  children?: React.ReactNode | ((props: { value: unknown; error?: string; onChange: (value: unknown) => void; onBlur: () => void }) => React.ReactNode);
}
...
export const Form: React.FC<FormProps> = ({
  ...
  const validateField = useCallback((name: string, value: unknown, rules?: ValidationRule[]): string => {
...
export const Field: React.FC<FieldProps> = ({
  ...
  const { values, errors, touched, setFieldValue, setFieldTouched, setFieldError, validateField } = context;
  ...
  return (
    <div className={`${styles.field} ${error && isTouched ? styles.hasError : ''}`}>
      {label && (
        <label htmlFor={name} className={styles.label}>
          {label}
          {rules?.some(r => r.required) && <span className={styles.required}>*</span>}
        </label>
      )}
      ...
      {(error && isTouched) || hint ? (
        <span className={`${styles.hint} ${error && isTouched ? styles.errorHint : ''}`}>
    ...
    <Field name={name} label={label} hint={hint} rules={rules}>
      {({ value, error, onChange, onBlur }) => {
        ...
        const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
          const newValue = component === 'input' && type === 'checkbox'
