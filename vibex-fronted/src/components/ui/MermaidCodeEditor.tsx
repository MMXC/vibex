...
import type { OnMount } from '@monaco-editor/react'; // 类型定义使用 import type
      ...
      monaco.languages.setMonarchTokensProvider('mermaid', {
        tokenizer: {
          root: [
            ...
            // Keywords
            [
              /graph|flowchart|stateDiagram|classDiagram|sequenceDiagram|pie|gitGraph/,
              'keyword',
            ],
            [
              /subgraph|end|class|state|note|participant|actor|loop|alt|else|par|and|then|bar|queue/,
              'keyword',
            ],
  ...
  const validateMermaid = useCallback(
    (code: string) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (
          edgeMatch &&
          !line.match(new RegExp(`\\[${edgeMatch[1]}\\]|\\(${edgeMatch[1]}\\)`))
        ) {
      ...
      if (bracketCount !== 0) {
        errors.push(
          `Unbalanced square brackets: ${bracketCount > 0 ? 'missing ]' : 'missing ['}`
        );
      }

      if (parenCount !== 0) {
        errors.push(
          `Unbalanced parentheses: ${parenCount > 0 ? 'missing )' : 'missing ('}`
        );
      }
  ...
  return (
    <div
      style={{
        borderRadius: '8px',
        overflow: 'hidden',
        border: errors.length > 0 ? '1px solid #f87171' : '1px solid #3b3b5c',
      }}
    >
      <Editor
        height={height}
