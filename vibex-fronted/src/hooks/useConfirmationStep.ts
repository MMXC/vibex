...
// 步骤顺序
export const STEPS: StepKey[] = ['input', 'context', 'model', 'flow', 'success'];
...
export function useConfirmationStep(): UseConfirmationStepReturn {
  ...
  const currentStep: StepKey = STEPS.includes(urlStep as StepKey) 
    ? (urlStep as StepKey) 
    : (STEPS.includes(currentStepFromStore as StepKey) ? currentStepFromStore as StepKey : 'input');
  ...
  // 检查步骤是否完成
  const isStepCompleted = useCallback((step: StepKey): boolean => {
  ...
  // 跳转到指定步骤
  const goToStep = useCallback((step: StepKey | number) => {
