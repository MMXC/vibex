...
export function usePermission(): UsePermissionReturn {
  ...
  useEffect(() => {
    ...
    if (cached) {
      try { setProjectRoles(JSON.parse(cached)); } catch { /* ignore */ }
    }
  }, []);
  ...
  const canAccess = useCallback((resource: Resource, permission: Permission): boolean => {
    if (globalRole === 'super_admin') return true;
    if (globalRole === 'user' && permission === 'manage') return false;
    return hasPermission(permission);
  }, [globalRole, hasPermission]);

  const refreshUser = useCallback(() => { window.location.reload(); }, []);
  ...
  const getProjectRole = useCallback((projectId: string): ProjectRole | null => {
    ...
    if (cached.expiresAt < Date.now()) {
      ...
      if (typeof window !== 'undefined') localStorage.setItem('project_roles', JSON.stringify(newRoles));
      return null;
    }
    return cached.role;
  }, [projectRoles]);
  ...
  const hasProjectPermission = useCallback(async (projectId: string, permission: Permission): Promise<boolean> => {
    ...
    if (cachedRole) {
      if (globalRole === 'super_admin' || cachedRole === 'owner' || cachedRole === 'admin') return true;
      return PROJECT_ROLE_PERMISSIONS[cachedRole]?.includes(permission) ?? false;
    }
    try {
      const response = await apiService.getProjectRole(projectId);
      if (response && response.role) {
        const newRoles = { ...projectRoles, [projectId]: { role: response.role, expiresAt: Date.now() + 86400000 } };
        setProjectRoles(newRoles);
        if (typeof window !== 'undefined') localStorage.setItem('project_roles', JSON.stringify(newRoles));
        if (globalRole === 'super_admin' || response.role === 'owner' || response.role === 'admin') return true;
        return PROJECT_ROLE_PERMISSIONS[response.role]?.includes(permission) ?? false;
      }
    } catch (error) { console.error('Failed to get project role:', error); }
    return false;
  }, [globalRole, projectRoles, getProjectRole]);

  const setProjectRoleFn = useCallback((projectId: string, role: ProjectRole): void => {
    const newRoles = { ...projectRoles, [projectId]: { role, expiresAt: Date.now() + 86400000 } };
    setProjectRoles(newRoles);
    if (typeof window !== 'undefined') localStorage.setItem('project_roles', JSON.stringify(newRoles));
  }, [projectRoles]);

  const clearProjectRoleFn = useCallback((projectId: string): void => {
    const newRoles = { ...projectRoles };
    delete newRoles[projectId];
    setProjectRoles(newRoles);
    if (typeof window !== 'undefined') localStorage.setItem('project_roles', JSON.stringify(newRoles));
  ...
  return {
    globalRole, isAuthenticated, isSuperAdmin, isUser, isGuest,
    hasPermission, canAccess, user, refreshUser,
    getProjectRole, hasProjectPermission, setProjectRole: setProjectRoleFn, clearProjectRole: clearProjectRoleFn,
  };
}

export function checkGlobalRolePermission(role: GlobalRole, permission: Permission): boolean {
  if (role === 'super_admin') return true;
  return GLOBAL_ROLE_PERMISSIONS[role]?.includes(permission) ?? false;
}

export function checkProjectRolePermission(role: ProjectRole, permission: Permission): boolean {
...
export function getGlobalRoleDisplayName(role: GlobalRole): string {
  const names: Record<GlobalRole, string> = { super_admin: '超级管理员', user: '用户', guest: '访客' };
  return names[role] || '未知';
}

export function getProjectRoleDisplayName(role: ProjectRole): string {
  const names: Record<ProjectRole, string> = { owner: '所有者', admin: '管理员', editor: '编辑者', viewer: '查看者' };
  return names[role] || '未知';
}

export function getRoleColor(role: UserRole): string {
  const colors: Record<UserRole, string> = {
    super_admin: '#ef4444', user: '#3b82f6', guest: '#6b7280',
    owner: '#8b5cf6', admin: '#f59e0b', editor: '#10b981', viewer: '#6b7280',
  };
