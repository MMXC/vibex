...
test.describe('Vibex E2E Tests', () => {
  ...
  test('登录功能', async ({ page }) => {
    ...
    // 输入邮箱
    await page.fill('input[type="email"], input[name="email"], input[placeholder*="email"]', 'y760283407@outlook.com');
    // 输入密码
  ...
  test('仪表盘页面', async ({ page }) => {
    ...
    // 检查仪表盘内容
    await expect(page.locator('h1')).toContainText(/我的项目|Dashboard|仪表盘|工作台/i, { timeout: 5000 });
  });
});
